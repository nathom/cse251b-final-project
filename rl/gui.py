...
from pygame.locals import K_RETURN, KEYDOWN, QUIT, K_e
...
class GameRunner:
    ...
    def print_matrix(self):
        ...
        for i in range(0, self.game.board_size):
            for j in range(0, self.game.board_size):
                ...
                if self.grayscale:
                    color_order = (
                        min(math.log(tile_count, 2), MAX_CORD) if tile_count > 0 else 0
                    )
                ...
                pygame.draw.rect(
                    self.surface, color, (ul_x_px, ul_y_px, tile_size, tile_size)
                )
                pygame.draw.rect(
                    self.surface, BLACK, (ul_x_px, ul_y_px, tile_size, tile_size), 2
                )
    ...
    def getScoreLabel(self):
        return self.scorefont.render(
            "Score: {}".format(self.game.score), 1, BLACK, WHITE
        )
