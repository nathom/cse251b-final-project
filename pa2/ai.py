        ...
        for i in range(n):
            for j in range(n):
                if j < n - 1 and matrix[i][j] == matrix[i][j+1]:
                    ...
                    merge_count += 1
                elif j < n - 2 and matrix[i][j] == matrix[i][j+2] and all(elem == 0 for elem in matrix[i][j+1:j+2]):
                    # Cells are in the same row with a zero cell in between
                    merge_count += 1
                elif i < n - 2 and matrix[i][j] == matrix[i+2][j] and all(elem == 0 for elem in [matrix[i+1][j], matrix[i+2][j]]):
